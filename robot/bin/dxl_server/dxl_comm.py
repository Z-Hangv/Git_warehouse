...
from tool import *
import dxl_server.servo as servo
...
class Dxl:
    ...
    def load_config(self):
        ...
        for servo_name in servo_name_list:
            new_servo = servo.DxlServo(servo_name)
    ...
    def preset(self, word_list_):  # preset命令集
        ...
        if word_list_[3] == "set_torque":
            ...
            if set_value > 1023:
                m["msg"] = my_servo.name + " 转矩大小不能超过1023"
                return m
            my_servo.set_torque(set_value)  #
            m["success"] = 1
            m["msg"] = my_servo.name + "设定转矩大小位: %d" % set_value
            m["torque"] = set_value
            return m
