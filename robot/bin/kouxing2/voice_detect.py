...
for i in range(1) :
    f = wave.open(xbase.get_sound_file_path("1.wav"),"rb")
    ...
    with open("2_en_tmp.txt", "w") as f :
        for en in energy :
            f.write(str(en) + "\n")
    zeroCrossingRate = calZeroCrossingRate(wave_data)
    with open("2_zero_tmp.txt","w") as f :
    ...
    time.sleep(1)
    print(1)
    with open("2_out_tmp.txt","w") as f :
        for zcr in zeroCrossingRate :
            count = count + 1
            zcr_sum = zcr_sum +zcr
            time.sleep(15/10000)
            threshold = 0.03
            if count > 10000*0.2/15:
                if zcr_sum < threshold * count:
                    f.write('close mouth' + "\n")
                    if speaking:
                        # print("time lasp "  + str(time.time() - time_tick))
                        time_tick = time.time()
                        speaking =False
                        print('speaking')
